...
| Remote version older than local (developer rolled back a release) | **Forward-only.** `:compare_versions` uses PowerShell's `[version]` ordering, and an update runs only when the remote is *strictly newer*. The silent path exits 0; the interactive path says the install is newer than the published release and leaves it alone. Downgrading is deliberate: delete the install folder and re-run the installer. |
| Version string unparseable on either side | `VERSION_COMPARE_FAILED=1`, no update, reason logged. "Cannot tell" must never authorise overwriting an install. The one exception is the interactive path with a missing/empty local `version.txt`, which refreshes — that install is already broken and user data is preserved. |
| `setup.bat --silent-update` run inside a git clone | **Skipped entirely** (`.git` present next to `setup.bat`), before any network call, reason logged. Developers update through git; extracting a release zip over a working tree destroys uncommitted work — and would do so on a legitimate upgrade just as readily as on a downgrade. |
